...
.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DelayHistoryAdapter.ViewHolder {
    ...
    override fun onBindViewHolder(holder: DelayHistoryAdapter.ViewHolder, position: Int) {
        ...
        when(eWork.machineType){
            1 -> { holder.itemView.eth_machine_type.setText(":  Excavator")}
            2 -> { holder.itemView.eth_machine_type.setText(":  Scrapper")}
            3 -> { holder.itemView.eth_machine_type.setText(":  Truck")}
        }
