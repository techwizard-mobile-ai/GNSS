...
class CSVWriter @JvmOverloads constructor(
  writer: Writer?, separator: Char = CSVWriter.Companion.DEFAULT_SEPARATOR, quotechar: Char = CSVWriter.Companion.DEFAULT_QUOTE_CHARACTER, escapechar: Char =
    CSVWriter.Companion.DEFAULT_ESCAPE_CHARACTER, lineEnd: String = CSVWriter.Companion.DEFAULT_LINE_END
) {
   ...
   * entry.
   */
  fun writeNext(nextLine: ArrayList<String>) {
    ...
    for (i in nextLine.indices) {
      ...
      if (quotechar != CSVWriter.Companion.NO_QUOTE_CHARACTER) sb.append(quotechar)
      for (j in 0 until nextElement.length) {
        val nextChar = nextElement[j]
        if (escapechar != CSVWriter.Companion.NO_ESCAPE_CHARACTER && nextChar == quotechar) {
          sb.append(escapechar).append(nextChar)
        } else if (escapechar != CSVWriter.Companion.NO_ESCAPE_CHARACTER && nextChar == escapechar) {
          sb.append(escapechar).append(nextChar)
        } else {
          sb.append(nextChar)
        }
