          ...
          if (responseBody!!.success) {
            ...
            getServerSyncData.forEach { serverSyncAPI ->
              when (serverSyncAPI.type) {
                ...
                18 -> {
                  myHelper.log("${serverSyncAPI.type}--${serverSyncAPI.name}--${serverSyncAPI.myDataList.size}")
                  db.insertMvpOrgsProjects(serverSyncAPI.myDataList)
                }
                20 -> {
                  myHelper.log("${serverSyncAPI.type}--${serverSyncAPI.name}--${serverSyncAPI.myDataList.size}")
                  db.insertAdminMvpSurveysLabels(serverSyncAPI.myDataList)
                }
              }
            }
...
//        This method is called when Mode is changed. So Reset Start Time of Operator Hour
  ...
  fun checkUpdateServerSyncDataCoroutine(type: Int) {
    ...
    addToList(14, myHelper.getTypeName(14), db.getMvpOrgsFiles(MyEnum.ADMIN_FILE_TYPE_TAPU_SURVEY_POINT, "ASC"))?.let { serverSyncList.add(it) }
    addToList(15, myHelper.getTypeName(15), db.getMvpOrgsFiles(MyEnum.ADMIN_FILE_TYPE_TAPU_SURVEY_LINE, "ASC"))?.let { serverSyncList.add(it) }
